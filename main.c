#include "scheduler.h"
#include <string.h>
#include <unistd.h> // For trace suppression
#include <fcntl.h>  // For trace suppression
...
void print_input_data(Process p[], int n, char* title) {
    printf("\n>>> LOADED SCENARIO: %s\n", title);
    printf("---------------------------------------------------\n");
    printf("PID | Arrival | Burst | Priority (1=High)\n");
    printf("---------------------------------------------------\n");
    for(int i=0; i<n; i++) {
        printf(" %-2d | %-7d | %-5d | %-5d\n", 
...
void calculate_all_stats(Process p_orig[], int n) {
    ...
    // Redirect stdout to NULL to silence the algorithms
    ...
    switch(choice) {
        case 1: 
            *n = 4; strcpy(name_buffer, "Standard Mix");
            p[0] = (Process){1, 0, 8, 2, 8}; p[1] = (Process){2, 1, 4, 1, 4};
            p[2] = (Process){3, 2, 9, 3, 9}; p[3] = (Process){4, 3, 5, 2, 5};
            break;
        case 2: 
            *n = 3; strcpy(name_buffer, "Convoy Effect (FCFS Worst Case)");
            p[0] = (Process){1, 0, 100, 3, 100}; p[1] = (Process){2, 1, 1, 1, 1};
            p[2] = (Process){3, 2, 1, 1, 1};
            break;
        case 3: 
            *n = 3; strcpy(name_buffer, "Reverse Convoy (SJF Best Case)");
            p[0] = (Process){1, 0, 2, 1, 2}; p[1] = (Process){2, 1, 3, 1, 3};
            p[2] = (Process){3, 2, 100, 3, 100};
            break;
        case 4: 
            *n = 5; strcpy(name_buffer, "Batch Arrival (All t=0)");
            p[0] = (Process){1, 0, 10, 3, 10}; p[1] = (Process){2, 0, 1, 1, 1};
            p[2] = (Process){3, 0, 2, 3, 2};   p[3] = (Process){4, 0, 1, 4, 1};
            p[4] = (Process){5, 0, 5, 2, 5};
            break;
        case 5: 
            *n = 3; strcpy(name_buffer, "Priority Dominance");
            p[0] = (Process){1, 0, 10, 3, 10}; p[1] = (Process){2, 2, 4, 2, 4};
            p[2] = (Process){3, 4, 2, 1, 2};
            break;
        case 6: 
            *n = 4; strcpy(name_buffer, "Equal Load (Fairness Test)");
            p[0] = (Process){1, 0, 4, 1, 4}; p[1] = (Process){2, 1, 4, 1, 4};
            p[2] = (Process){3, 2, 4, 1, 4}; p[3] = (Process){4, 3, 4, 1, 4};
            break;
        case 7: 
            *n = 3; strcpy(name_buffer, "MLFQ Stress Test");
            p[0] = (Process){1, 0, 20, 1, 20}; p[1] = (Process){2, 4, 2, 1, 2};
            p[2] = (Process){3, 8, 4, 1, 4};
            break;
        case 8: 
            *n = 5; strcpy(name_buffer, "Starvation Risk");
            p[0] = (Process){1, 0, 20, 5, 20}; p[1] = (Process){2, 1, 2, 1, 2};
            p[2] = (Process){3, 2, 2, 1, 2};   p[3] = (Process){4, 3, 2, 1, 2};
            p[4] = (Process){5, 4, 2, 1, 2};
            break;
        case 9: 
            *n = 3; strcpy(name_buffer, "MLQ System vs User");
            p[0] = (Process){1, 0, 10, 1, 10}; p[1] = (Process){2, 0, 5, 2, 5};
            p[2] = (Process){3, 2, 8, 1, 8};
            break;
        case 10: 
            *n = 5; strcpy(name_buffer, "Complex Random Workload");
            p[0] = (Process){1, 0, 12, 3, 12}; p[1] = (Process){2, 3, 6, 2, 6};
            p[2] = (Process){3, 5, 3, 1, 3};   p[3] = (Process){4, 10, 8, 2, 8};
            p[4] = (Process){5, 12, 2, 1, 2};
            break;
    }
}
...
int main() {
    ...
    while(1) {
        printf("\n=============================================\n");
        printf("   CPU SCHEDULING ALGORITHM SELECTOR (v2.0)  \n");
        printf("=============================================\n");
        ...
        if (choice == 1) {
            ...
            for(int i=0; i<n; i++) {
                ...
                printf("P%d Arrival: ", i+1); scanf("%d", &master_procs[i].arrival_time);
                printf("P%d Burst: ", i+1); scanf("%d", &master_procs[i].burst_time);
                printf("P%d Priority: ", i+1); scanf("%d", &master_procs[i].priority);
                master_procs[i].remaining_time = master_procs[i].burst_time;
            }
            print_input_data(master_procs, n, scenario_name);
        }

        // 2. RUN ANALYSIS (Silent)
        ...
        while(1) {
            ...
            if(sub==1) calculate_fcfs(temp_procs, n);
                ...
                printf("Enter Quantum for Round Robin execution: ");
                int q; scanf("%d", &q);
